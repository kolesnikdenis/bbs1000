...
function readstatolt($str,$stat_olt){
	...
	while ($i <= $count)	{
		...
		$find = strripos($arry1[2],":");
		if ( (strlen($arry1[2])>10 ) && (  $find > 1) ) {
			$olt=$arry1[1];
			$onustat[$stat_olt] .= "<tr><td>Record</td><td>".$olt[0] ."</td></tr>";
			$onustat[$stat_olt] .= "<tr><td>OLT</td><td>".$olt[1].$olt[2].$olt[3]."</td></tr>";
			$t = $olt[1].$olt[2].$olt[3];
			$onustat[$stat_olt] .= "<tr><td class=red>Mac_Address</td><td class=red>".$arry1[2]."- <a href=\"find_free_onu.php?epon_num=".$t."&macc=".$arry1[2]."\"> register</a></td></tr>";
		}
		$i++;
	} // while
