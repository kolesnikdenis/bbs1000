...
$epon_num = $_GET['epon_num'];
 ...
 if ($result === true){
	 ...
	 echo "<tr><td>show interface epon-olt ".$epon_num." onu-unbounda</td></tr>";
	 send_command($socket, 'show interface epon-olt '.$epon_num. ' onu-unbound');
	 
	 global $read_continiue;
	 echo "<select name=\"select\">";
	 onu_unbound($read_continiue,$epon_num);
	 echo "</select>";
...
function onu_unbound($str,$stat_olt){
	global $onustat;
	$out="";
	...
	while ($i <= $count-1)  {
		...
		$arry1 = explode(" ",$s);
		$olt=$arry1[1];
		if ( strlen($arry1[2])>10){
			$onustat[$stat_olt] .= "Record: ".$olt[0] ." \r\n";
			$onustat[$stat_olt] .= "OLT: ".$olt[1].$olt[2].$olt[3]." \r\n";
			$onustat[$stat_olt] .= "MAC:".$arry1[2]." \r\n";
			$options_unb[$stat_olt].= "<option value=\"".$arry1[2]."\">".$arry1[2]."</option>\r\n";
		}
		$i++;
	} // while
	$onustat[$stat_olt].= "\r\n ";
	#echo $onustat[$stat_olt]."\r\n";
	echo $options_unb[$stat_olt];
}
...
//		var_dump($str);
		...
		$out .="<br>free onu:<br>";
		$free= "<br>free onu:<br>";
		while ($i < 64){ if (@$arr_free[$i] == "") { $out.="<a href=\"add_olt.php?olt=$olt&onu=$i\">$i</a>, "; $free.="<a href=\"add_olt.php?olt=$olt&onu=$i\">$i</a>, ";} $i++;}
		return $out;
}
